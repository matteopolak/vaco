//! HEVC header parsing against arbitrary bytes.
//!
...
//! to the VPS, SPS, PPS, slice-header and SEI parsers as well, so a unit the
//! splitter would have skipped still gets parsed.
//!
...
//!   whole-buffer target — `vaco-parse-h264`'s fuzzer found three separate bugs
//!   against exactly this and none of them was reachable any other way.
//! * **Access units partition the input.** Every byte the parser emits came from
//!   the input, in order, exactly once.
//! * **Progress.** A call that consumes nothing and emits nothing is a hang; the
//!   target asserts the parser always consumes what it is given.
//! * **Derived geometry is self-consistent.** A reported width never exceeds the
//!   coded one, and neither is ever zero.
//!
...
use vaco_parse_hevc::{
    HevcDecoderConfigurationRecord, HevcNalHeader, HevcParser, NalUnitType, ParameterSets,
    Pps, SliceHeader, Sps, Vps, codec_parameters, params, sei, slice,
};
    ...
    for c in data.chunks(chunk.max(1)) {
        ...
        while !rest.is_empty() {
            match parser.parse(rest) {
                ...
                // A budget cap is correct behaviour; stop rather than continuing
                // with a parser whose buffer was refused.
                Err(_) => return out,
            }
        }
    }
...
fuzz_target!(|data: &[u8]| {
    ...
    // subsequence of the stream: leading garbage and empty units are dropped, so
    // this is containment rather than equality.
    assert!(
        joined.len() <= data.len(),
    ...
    for nal in units(data, Framing::AnnexB).take(4096) {
        ...
        let Some(header) = HevcNalHeader::parse(rbsp) else {
            continue;
        };
        match header.nal_unit_type {
            NalUnitType::VPS_NUT => {
                if let Ok(vps) = Vps::parse(rbsp, &mut budget) {
                    let _ = vps.frame_rate();
                }
                let _ = sets.add_vps(rbsp, &mut budget);
            }
            NalUnitType::SPS_NUT => {
                if let Ok(sps) = Sps::parse(rbsp, &mut budget) {
                    ...
                    if let Some((w, h)) = sps.dimensions() {
                        assert!(w > 0 && h > 0, "a zero dimension escaped the check");
                        assert!(w <= sps.coded_width(), "the window widened the picture");
                        assert!(h <= sps.coded_height(), "the window heightened it");
                    }
                    ...
                    let _ = sps.max_num_reorder_pics();
                    let _ = sps.pic_size_in_ctbs();
                    let _ = sps.slice_address_bits();
                    let _ = p.validate(&budget);
                }
                let _ = sets.add_sps(rbsp, &mut budget);
            }
            NalUnitType::PPS_NUT => {
                let _ = Pps::parse(rbsp, &mut budget);
    ...
    // ---- The same bytes as an `hvcC` record and as a length-prefixed sample.
    let mut budget = Budget::new(Limits::strict());
    if let Ok(record) = HevcDecoderConfigurationRecord::parse(data, &mut budget) {
        let _ = record.mime_codec_string();
        assert_eq!(
            record.vps().count() + record.sps().count() + record.pps().count(),
            record
                .arrays
                .iter()
                .filter(|a| a.nal_unit_type.is_parameter_set())
                .map(|a| a.units.len())
                .sum::<usize>(),
            "the typed accessors and the arrays must agree"
        );
        let mut parser = HevcParser::new(Limits::strict());
