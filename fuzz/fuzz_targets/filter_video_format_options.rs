...
//! `vaco_pixfmt::PixFmt::from_name`; `setsar`/`setdar` hand their ratio
...
//! vocabularies (`round`, `eof_action`). All of it runs on
//! `-filter_complex` text, matching the pattern in `vaco-filter-audio`'s
//! `filter_audio_options.rs`.
//!
...
const NAMES: &[&str] = &["format", "fps", "framerate", "setdar", "setfield", "setrange", "setsar"];
