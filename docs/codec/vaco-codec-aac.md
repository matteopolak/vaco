...
### Channel-configuration coverage: 1 through 7 direct; 0 via PCE; 11/12/14 gated
...
(5.1), and 7 (7.1) directly. Each has a specified `SCE`/`CPE`/`LFE` element
sequence. `channelConfiguration == 0` instead resolves exactly from a real
PCE via [`DecoderConfig::try_resolve_pending`].
...
#### Direct 7.1: element and plane order checked against a real control

Configuration 7 has raw syntax order `FC, FL, FR, SL, SR, BL, BR, LFE`; Vaco
permutes that to its native `FL, FR, FC, LFE, BL, BR, SL, SR` order. A fresh
1.024-second, 48 kHz, eight-tone AAC-LC ADTS control generated by `ffmpeg
9.0.1 -c:a aac` contained 49 packets and used a leading PCE for
...
`decode_dump` each emitted **1,572,864 bytes** (48 × 1024 × 8 × 4). The eight
per-plane correlations were 0.999998664 or higher, with no off-diagonal
permutation, which catches the otherwise plausible side/front-pair swap.

**Configurations 11, 12 and 14 remain rejected with `Error::Unsupported`.**
...
- **`coupling_channel_element()` (`CCE`) is not implemented** —
  ...
  Rare in real 1/2/3/4/5/6/7-channel content (this crate's resolved
  configurations); gated rather than guessed at.
- **`channelConfiguration` 11, 12, 14** are gated (see "Channel-
  ...
  output permutation for 1 through 7 for the same reason.
...
- **Adding a `channelConfiguration` value (11/12/14):** get ISO/IEC
  ...
  decoder needs), a unit test alongside the existing 1/2/3/4/5/6/7 cases, and a
  matching entry in `decoder.rs`'s `reorder_to_output_channel_order`. Do not
  extrapolate an ordering from the ones already here — 1/2/3/4/5/6/7 were chosen
