...
### Channel-configuration coverage: 1 through 7 and 12 direct; 0 via PCE; 11/14 gated
...
(5.1), 7 (7.1), and 12 (7.1) directly. Each has a specified
...
#### Direct 7.1: configurations 7 and 12 checked against real controls

Configurations 7 and 12 have raw syntax order `FC, FL, FR, SL, SR, BL, BR,
LFE`; Vaco permutes that to its native `FL, FR, FC, LFE, BL, BR, SL, SR` order.
A fresh 1.024-second, 48 kHz, eight-tone AAC-LC ADTS control generated by
`ffmpeg 9.0.1 -c:a aac` contained 49 packets and used a leading PCE for
`7.1(wide)`, so it could not itself exercise ADTS's direct three-bit value 7.
The first PCE frame was excluded and the remaining 48 PCE-free frames had only
their ADTS channel-configuration bits normalized to 7; `ffprobe` then reported
AAC-LC, 48 kHz, eight channels, `7.1`, and 48 packets. `ffmpeg -bitexact` and
Vaco each emitted **1,572,864 bytes** (48 × 1024 × 8 × 4). The eight per-plane
correlations were 0.999998664 or higher, with no off-diagonal permutation.

The MP4 control for configuration 12 is directly generated, not header
normalized: the same eight tones encoded by `ffmpeg 9.0.1 -c:a aac` produced
ASC `11 e0 56 e5 00`, 49 raw access units totalling 65,883 bytes, and an MP4 stream that
`ffprobe` identified as AAC-LC, 48 kHz, eight-channel `7.1`. Vaco's
configuration regression parses the complete ASC, while the black-box run fed
each MP4 access unit under that indicated configuration; it retained the first
AU's overlap history and excluded only its priming output, matching the MP4
presentation duration.
`ffmpeg -bitexact` and Vaco again emitted **1,572,864 bytes**. The eight
per-plane correlations were 0.999998844 or higher; the full cross-channel
matrix had one diagonal match per plane, so a side/front-pair swap cannot pass
on channel count alone.

**Configurations 11 and 14 remain rejected with `Error::Unsupported`.**
Their Table 42 element ordering still lacks a matching independent acceptance
control; the ISO/IEC 14496-26 vector set remains the closure criterion rather
...
- **`coupling_channel_element()` (`CCE`) is not implemented** —
  ...
  Rare in real 1/2/3/4/5/6/7/12-channel content (this crate's resolved
  configurations); gated rather than guessed at.
- **`channelConfiguration` 11 and 14** are gated (see "Channel-
  ...
  output permutation for 1 through 7 and 12 for the same reason.
...
- **Adding a `channelConfiguration` value (11/14):** get ISO/IEC
  ...
  decoder needs), a unit test alongside the existing 1/2/3/4/5/6/7/12 cases, and a
  matching entry in `decoder.rs`'s `reorder_to_output_channel_order`. Do not
  extrapolate an ordering from the ones already here — 1/2/3/4/5/6/7/12 were chosen
