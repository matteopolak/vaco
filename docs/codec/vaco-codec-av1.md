...
| `frame_header.rs` | `FrameHeader::parse`/`parse_from_reader` — intra syntax plus the bounded inter header prefix through single-reference skip-mode and identity global-motion syntax; it retains CDEF/restoration/loop-filter state for scope checks and refuses unsupported compound references, non-identity global motion, or later tile block modes by name |
...
- **Extending superres to inter frames**: the decoder already retains
  ...
  `superres_params()`. It also reaches the active P frame's single-reference
  skip/global-motion header form, then refuses before `inter_frame_mode_info`
  can read a motion vector. Add the matching pixel store, inter CDFs,
  motion-vector parsing, and block prediction at that established boundary;
  do not replace it with inferred geometry or a whole-frame copy.
