...
`senc` IV. `tenc` versions are limited to the defined version-0 and version-1
layouts, so an unknown version cannot be misread as version 1's pattern fields.
...
* ~~**HEIF/AVIF item model.**~~ Box-layer parsing done 2026-08-28 (`heif`),
  ...
  *HEIF/AVIF* section for what was measured. `ItemInfo` gained the
  ...
  by a tile-grid caller. The item resolver requires one `ispe` per image, and
  a grid's output dimensions must match its `ImageGrid` descriptor. Half-pixel
  centre offsets are retained when the resulting edges are integral, while zero
  denominators, fractional edges and
