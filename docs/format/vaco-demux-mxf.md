...
track's native edit rate. `duration_exact()` keeps the resulting rational
seconds without an intermediate microsecond conversion; the legacy
`duration()` view is derived from it with round-to-nearest for compatibility.
