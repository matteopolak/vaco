...
`duration()` and `duration_exact()`. Display formatting may round to
microseconds, but the duration value itself remains on the 100-nanosecond
clock for exact comparison and rescaling.
