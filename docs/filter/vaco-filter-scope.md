...
| Filter | Why |
|---|---|
| `oscilloscope` | **Still not shipped, but `sc`'s geometry is now fully pinned: it is one rotating dashed LINE, not a box or parallelogram.** The earlier "dashed parallelogram" read conflated the line with the always-on trace; isolating them (`c=0` removes the trace) showed every `sc=1` pixel at any `t` lies on exactly one straight line. Measured exactly: `angle = (0.5-t)*pi` (vertical at `t=0`/`1`, horizontal at `t=0.5`, the diagonals at `t=0.25`/`0.75`); `half_length = s*sqrt(w^2+h^2)/2`, clipped at the frame edge (confirmed proportional to the frame *diagonal*, not `w`/`h`, at five `s` values); `centre = (round(x*w)-1, round(y*h)-1)`, the same bias `pixscope`'s `window_start` uses. **The dash-phase rule** (period `2` pixels, `255`/`0` alternating) is anchored to the line's own geometric endpoint, not absolute frame-pixel parity — confirmed by shifting the line's start row by fractional amounts and finding the first pixel is always `255` regardless of that row's parity — and the `255` end is always the one reached by walking `+half_length` in direction `(cos(angle), -sin(angle))` from centre, confirmed across the vertical, horizontal and one diagonal case. The trace itself is still open: a real render (a `gradients` source, `c=1`) shows a semi-transparent dark box (source values drop from `~100-143` to `~32-41` under it, consistent with `o`'s `0.8` default opacity) with a bright trace spiking through — the blend formula, box background colour, and what source pixels feed the trace are all unmeasured. Shipping the `sc` line alone while erroring on the default `c=7` would misrepresent the filter more than help, so this stays characterised, not shipped — but the geometry is now solid enough to implement mechanically once the trace itself is cracked. |
| `ciescope` | **Not a D7 case, and the sourcing question this dispatch specifically asked about is resolved: the underlying data is honestly sourceable.** `-h filter=ciescope` shows it needs the CIE 1931 standard observer colour-matching functions (for the spectral-locus curve) plus published primary sets (BT.709/BT.2020/DCI-P3/SMPTE-C/NTSC/Adobe Wide Gamut/…, `system`'s ten values) and three coordinate transforms (`xyY`/UCS/`Luv`, `cie`'s three values) — all public, standardised, non-authorial data, the same footing BT.709/BT.2020's primaries already stood on for `vaco-filter-color`. This is *not* `hqx`'s situation, or the `#360`-style "reconstruct a table from memory" trap: nothing here needs guessing at data nobody published. **The real blocker is unchanged from the previous pass and is a rendering-measurement one, not a data one**: the spectral-locus curve's rasterisation, the gamut-triangle line drawing, `fill`'s CIE-colour computation, `gamma`/`corrgamma`/`contrast`'s exact application, and the source-pixel histogram accumulation (`intensity`, `showwhite`) are none of them specified by the standards themselves — reproducing them needs the same kind of extensive black-box probing `vectorscope`'s histogram needed, but across a meaningfully larger surface (three coordinate spaces, ten gamuts, a curve instead of a per-cell grid). Not attempted this pass for exactly that reason: the data is available, but doing the rendering measurement justice was larger than this dispatch's remaining time. |
