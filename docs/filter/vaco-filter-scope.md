...
| Filter | Why |
|---|---|
| `oscilloscope` | **Still not shipped, but `sc`'s geometry is now fully pinned: it is one rotating dashed LINE, not a box or parallelogram.** The earlier "dashed parallelogram" read conflated the line with the always-on trace; isolating them (`c=0` removes the trace) showed every `sc=1` pixel at any `t` lies on exactly one straight line. Measured exactly: `angle = (0.5-t)*pi` (vertical at `t=0`/`1`, horizontal at `t=0.5`, the diagonals at `t=0.25`/`0.75`); `half_length = s*sqrt(w^2+h^2)/2`, clipped at the frame edge (confirmed proportional to the frame *diagonal*, not `w`/`h`, at five `s` values); `centre = (round(x*w)-1, round(y*h)-1)`, the same bias `pixscope`'s `window_start` uses. **The dash-phase rule** (period `2` pixels, `255`/`0` alternating) is anchored to the line's own geometric endpoint, not absolute frame-pixel parity — confirmed by shifting the line's start row by fractional amounts and finding the first pixel is always `255` regardless of that row's parity — and the `255` end is always the one reached by walking `+half_length` in direction `(cos(angle), -sin(angle))` from centre, confirmed across the vertical, horizontal and one diagonal case. **The trace's sampling rule is now confirmed structurally**: fed a source whose value varies by row only (constant across each row), the trace is invisible at a horizontal scan (`t=0.5`, since sampling that row gives the same value the background already has) but shows a genuine bright spike at a tilted/vertical scan (`t=0.25`/`t=0`) — a fixed-row sample could not produce that asymmetry. **The trace samples along the tilted `sc` scan line's own geometry**, not a fixed row or column. Still open: the exact column-to-line-position mapping, whether `tx`/`ty`/`tw`/`th` only relocates the drawn box or also redefines the sampling geometry independently of `x`/`y`/`s`/`t`, and the blend arithmetic (a two-point fit lands near `opacity≈0.8` toward a near-black `~15` background, matching most but not all measured points — recorded as an open residual, not rounded away). Shipping the `sc` line alone while erroring on the default `c=7` would misrepresent the filter more than help, so this stays characterised, not shipped — closer than before, with the sampling question answered structurally. |
| `ciescope` | **Not a D7 case — the sourcing question is resolved, and not `#360`'s situation**: the CIE 1931 standard observer and every named primary set (BT.709/BT.2020/DCI-P3/SMPTE-C/NTSC/Adobe Wide Gamut/…) are public, standardised, non-authorial data with none of `#360`'s "recalled from the standard or from an open-source decoder, indistinguishably" ambiguity. **New this pass**: the reference's own output is `512x512 rgba64le` by default (`s` ranges `256..8192`) — 16-bit RGBA, a format and bit depth no other filter in this crate needs — on top of the already-known rendering surface (spectral-locus rasterisation, gamut-triangle drawing, colour fill across three coordinate systems `xyY`/UCS/`Luv`). Not attempted: the combined new-format-plus-rendering-surface task is bigger than this pass's remaining time could measure responsibly, not a data-sourcing question. |
