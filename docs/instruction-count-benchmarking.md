...
| `xctrace` / Instruments "CPU Counters" | Records real PMU events, but the manually configured fixed-event template exports *sampled per-core* counters (`counters-profile` / `kdebug-counters-with-time-sample`), not a process total. Values reset when a target migrates between cores, so they must not be summed. Kept as a diagnostic, not as the A/B instrument. |
| Other installed Instruments templates | Not a counter workaround. On this Xcode 16.0 installation, CPU Profiler exposes sampled `CORE_ACTIVE_CYCLE` rows (`kdebug-counters-with-pmi-sample`) without a process aggregate; Time Profiler exposes no counter table; and Processor Trace refuses an ordinary command-line target without a debug entitlement. Signposts can delimit a target interval but cannot turn any of those samples into a process total. |
| `powermetrics --show-process-ipc` | **Candidate, not adopted.** The installed macOS manual says this reports per-process instructions and cycles on ARM. It requires superuser privileges; on 2026-09-05 the local `sudo -n` check required a password, so no counter output was sampled or parsed. Before adding a harness or reporting a ratio, validate a bounded target's named process row, counter delta and child-process accounting under the exact `sudo powermetrics --samplers tasks --show-process-ipc --show-process-amp --format plist` invocation, then run the normal 10-round rotated Vaco/ffmpeg protocol. |
