...
//!
//! Mirrors `vaco-filter-audio-eq::sample` and `vaco-filter-audio-dynamics::sample`
//! exactly: every filter in this crate needs to read and combine sample values
//! regardless of the twelve `SampleFmt` variants a link might have negotiated,
//! so this decodes a frame's planes to planar `f64` via
//! `vaco_resample::convert::convert` — the crate carrying the project's
//! measured numeric contract (D17) — and encodes back afterwards. Not shared
//! as a library between the sibling crates because each keeps it
//! `pub(crate)`; duplicating ~90 lines here is cheaper than a new shared crate
//! for one helper, and `cargo xtask dup-check` scans `pub` items, which this
//! module has none of outside its own crate.
...
pub(crate) fn decode(frame: &Frame) -> Result<(SampleFmt, u32, u32, ChannelLayout, Channels)> {
    ...
    let mut bufs: SmallVec<[Vec<u8>; 8]> = (0..channels)
        ...
        .collect();
    {
        // `Vec`, not `SmallVec`: see `vaco-filter-audio::sample` for why
        // `#[may_dangle]` on `Vec::drop` is load-bearing here.
