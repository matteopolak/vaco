...
pub fn composite_bitmap(frame: &mut Frame, rect: &SubtitleRect) -> Result<()> {
    let SubtitleContent::Bitmap {
        ...
        return Err(Error::Unsupported(
            "vaco-filter-subtitle::bitmap: rect is not a bitmap",
        ));
    };
    let FrameData::Video {
        format,
        ...
        return Err(Error::Unsupported(
            "vaco-filter-subtitle::bitmap: not a video frame",
        ));
    };
        ...
        for (logical, comp) in desc.components.iter().enumerate() {
            ...
            for py in prect.y..prect.y.saturating_add(prect.h) {
                ...
                for px in prect.x..prect.x.saturating_add(prect.w) {
                    ...
                    let Some(&index) = data
                        .as_slice()
                        .get(src_y as usize * stride + src_x as usize)
                    else {
                        continue;
                    };
                    let Some(solid) = resolved.get(index as usize) else {
                        continue;
                    };
                    ...
                    let Some(dst_v) = sample::read(row, px as usize, comp, big_endian) else {
                        continue;
                    };
                    let out = if is_alpha_channel {
                        let max = if comp.depth >= 32 {
                            u32::MAX
                        } else {
                            (1u32 << comp.depth) - 1
                        };
                        #[allow(
                            ...
                            reason = "a is a 0..=1 fraction and max bounds the result"
                        )]
...
fn blend_channel(dst: u32, src: u32, a: f64) -> u32 {
    #[allow(
        clippy::cast_possible_truncation,
        clippy::cast_sign_loss,
        reason = "a convex combination of two bounded code values"
    )]
    {
...
fn composite_alpha(src_a: u32, dst_a: u32, depth: u8) -> u32 {
    ...
    #[allow(
        clippy::cast_possible_truncation,
        clippy::cast_sign_loss,
        reason = "out is a convex combination of two 0..=1 fractions"
    )]
    {
...
mod tests {
    ...
    fn opaque_bitmap_paints_its_own_colour() {
        ...
        let rect =
            SubtitleRect::bitmap(&mut budget, 2, 2, 2, 2, false, 2, &pixels, palette).unwrap();
    ...
    fn transparent_index_leaves_the_frame_untouched() {
        ...
        let rect =
            SubtitleRect::bitmap(&mut budget, 0, 0, 1, 1, false, 1, &pixels, palette).unwrap();
        composite_bitmap(&mut f, &rect).unwrap();
    ...
    fn out_of_frame_rect_clips_cleanly() {
        ...
        let palette = vec![[0, 0, 0, 0], [255, 255, 255, 255]];
        let rect =
            SubtitleRect::bitmap(&mut budget, 100, 100, 1, 1, false, 1, &pixels, palette).unwrap();
        composite_bitmap(&mut f, &rect).unwrap();
    }
