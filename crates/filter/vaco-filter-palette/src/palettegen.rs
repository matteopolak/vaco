...
impl Opts {
    fn parse(args: Option<&str>) -> std::result::Result<Self, String> {
        ...
        if o.transparency_color != "lime" {
            return Err("palettegen: `transparency_color` is parsed but not applied by this crate; refusing rather than silently ignoring it".to_string());
        }
        Ok(o)
    }
}
