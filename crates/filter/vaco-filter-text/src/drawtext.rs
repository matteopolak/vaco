...
const XY_VARS: &[&str] = &[
    "w", "h", "main_w", "main_h", "text_w", "tw", "text_h", "th", "line_h", "lh", "x", "y", "n",
    "t",
];
...
#[allow(
    ...
    reason = "drawtext's own option surface is genuinely this many independent booleans"
)]
pub(crate) struct Opts {
    ...
    #[opt(
        name = "line_spacing",
        help = "set line spacing in pixels",
        default = 0,
        flags(video, filtering)
    )]
    ...
    #[opt(
        name = "fix_bounds",
        help = "check and fix text coords to avoid clipping",
        default = false,
        flags(video, filtering)
    )]
    pub fix_bounds: bool,
    ...
    #[opt(
        name = "rtl",
        help = "force right-to-left text rendering",
        default = false,
        flags(video, filtering)
    )]
    pub rtl: bool,
    #[opt(
        name = "reload",
        help = "reload text file at specified frame interval",
        default = false,
        flags(video, filtering)
    )]
    pub reload: bool,
...
impl Opts {
    fn parse(args: Option<&str>) -> std::result::Result<Self, String> {
        ...
        if o.rtl {
            return Err("drawtext: `rtl` is parsed but not applied by this crate; refusing rather than silently ignoring it".to_string());
        }
        Ok(o)
    }
}
...
#[allow(
    clippy::struct_excessive_bools,
    reason = "one flag per independent drawtext option, not a state machine"
)]
...
impl Filter {
    fn new(opts: &Opts) -> std::result::Result<Self, String> {
        ...
        let fontcolor = vaco_core::parse::color(&opts.fontcolor)
            .ok_or_else(|| format!("drawtext: bad fontcolor `{}`", opts.fontcolor))?;
        let boxcolor = vaco_core::parse::color(&opts.boxcolor)
            .ok_or_else(|| format!("drawtext: bad boxcolor `{}`", opts.boxcolor))?;
        let bordercolor = vaco_core::parse::color(&opts.bordercolor)
            .ok_or_else(|| format!("drawtext: bad bordercolor `{}`", opts.bordercolor))?;
        let shadowcolor = vaco_core::parse::color(&opts.shadowcolor)
            .ok_or_else(|| format!("drawtext: bad shadowcolor `{}`", opts.shadowcolor))?;
        ...
        Ok(Self {
            ...
            fontsize_expr: Expr::parse(&opts.fontsize, &Bindings::new(SIZE_VARS))
                .map_err(|e| format!("drawtext: bad fontsize `{e}`"))?,
            fontcolor,
            alpha_expr: Expr::parse(&opts.alpha, &Bindings::new(SIZE_VARS))
                .map_err(|e| format!("drawtext: bad alpha `{e}`"))?,
            ...
            x_expr: Expr::parse(&opts.x, &Bindings::new(XY_VARS))
                .map_err(|e| format!("drawtext: bad x `{e}`"))?,
            y_expr: Expr::parse(&opts.y, &Bindings::new(XY_VARS))
                .map_err(|e| format!("drawtext: bad y `{e}`"))?,
...
impl FrameFilter for Filter {
    fn filter_frame(&mut self, _ctx: &mut FilterContext<'_>, input: Frame) -> Result<FrameOut> {
        ...
        let (mut ox, mut oy) = anchor.place(
            x0 as f32,
            y0 as f32,
            layout.width as f32,
        ...
        if self.shadowx != 0 || self.shadowy != 0 {
            ...
            mask::composite(
                &mut out,
                &shadow_mask,
                scaled_alpha(self.shadowcolor, alpha_v),
                color_info,
            )?;
        }
        if self.borderw > 0 {
            let dilated =
                base_mask.dilate(self.renderer.budget_mut(), self.borderw.min(64) as u32)?;
            mask::composite(
                &mut out,
                &dilated,
                scaled_alpha(self.bordercolor, alpha_v),
                color_info,
            )?;
        }
        mask::composite(
            &mut out,
            &base_mask,
            scaled_alpha(self.fontcolor, alpha_v),
            color_info,
        )?;
