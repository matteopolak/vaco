...
//!
//! Mirrors `vaco-filter-audio`'s module of the same name: every dynamics
//! filter needs to read and combine sample values regardless of the twelve `SampleFmt`
//! variants a link might have negotiated, so this decodes a frame's planes to
//! planar `f64` via `vaco_resample::convert::convert` — the crate carrying the
//! project's measured numeric contract (D17) — and encodes back afterwards.
//! Not shared as a library between the two crates because `vaco-filter-audio`
//! keeps it `pub(crate)`; duplicating ~90 lines here is cheaper than a new
//! shared crate for one helper, and `cargo xtask dup-check` scans `pub`
//! items, which this module has none of outside its own crate.
