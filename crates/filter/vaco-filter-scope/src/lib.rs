...
//! | `pixscope` | Attempted, not shipped, but not for the reason `datascope` was blocked on: the font mechanism is confirmed shared (identical glyph family, no font option). What was newly measured and is *not* what its option names suggest: `pixscope`'s "zoom" box does not magnify anything. A `7x7`-source-pixel `o=1` window over a hand-built period-2 checkerboard shows the checkerboard's own `1:1` period unchanged inside a `1`px-bordered outline — it is a location marker, not a magnifier. The actual numeric payload lives entirely in a separate, much larger statistics text panel (position, per-component values, evidently min/max/average, auto-placed via the `wx`/`wy` `-1` sentinel), whose exact label set and layout this pass did not reverse-engineer. Shipping the marker box alone would not be the useful part of this filter; the text panel is, and it needs real measurement before a next attempt, not a guess. |
