//! `dilation` — greyscale morphological dilation. See [`crate::morph`] for
//! the shared engine and the measurements pinning down `coordinates` and
//! `threshold`.
...
#[options(name = "dilation", help = "Apply dilation effect")]
...
impl FrameFilter for Filter {
    fn filter_frame(&mut self, ctx: &mut FilterContext<'_>, input: Frame) -> Result<FrameOut> {
        ...
        for p in 0..format.plane_count() {
            ...
            let filtered = morph::apply_plane(&rows, pw, ph, Op::Dilate, params);
