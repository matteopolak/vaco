...
/// Parse a `planes` bitmask option (0-15): bit `i` selects logical channel
/// `i`. Every filter in this crate that has a `planes` option documents it
/// this way (`ffmpeg -h filter=maskedmerge` and friends).
#[must_use]
pub(crate) const fn plane_selected(mask: i64, channel: usize) -> bool {
    if channel >= 4 {
        return false;
    }
    (mask >> channel) & 1 != 0
...
mod tests {
    ...
    #[test]
    fn plane_mask_reads_each_bit() {
        assert!(plane_selected(0b0101, 0));
        assert!(!plane_selected(0b0101, 1));
        assert!(plane_selected(0b0101, 2));
        assert!(!plane_selected(0b0101, 3));
        assert!(!plane_selected(0b1111, 4));
    }
}
