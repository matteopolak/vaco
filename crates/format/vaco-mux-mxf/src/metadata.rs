...
use crate::localset::{
    push_batch16, push_i64, push_item, push_rational, push_u8, push_u16, push_u32, push_u64,
...
pub(crate) fn build_sets(
    ...
    push_item(
        &mut preface,
        0x3b09,
        &ul::operational_pattern_for(variant).as_bytes(),
    );
    push_batch16(
        &mut preface,
        0x3b0a,
        &essence_containers_used(tracks, variant),
    );
    ...
    for (track_uid, seq_uid, comp_uid, is_material) in [
        (
            ids.timecode.mp_track,
            ids.timecode.mp_sequence,
            ids.timecode.mp_component,
            true,
        ),
        (
            ids.timecode.sp_track,
            ids.timecode.sp_sequence,
            ids.timecode.sp_component,
            false,
        ),
    ] {
        let _ = is_material;
    ...
    if let Some(md_id) = ids.multiple_descriptor {
        ...
        push_item(
            &mut md,
            0x3004,
            &ul::ESSENCE_CONTAINER_MULTIPLE_WRAPPINGS.as_bytes(),
        );
...
fn display_aspect_ratio(sar: Rational, width: u32, height: u32) -> Option<(i32, i32)> {
    ...
    let dar = sar.checked_mul(Rational::new(
