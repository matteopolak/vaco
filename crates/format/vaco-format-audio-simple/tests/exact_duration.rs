...
use vaco_format_audio_simple::caf::CafDemuxer;
...
fn aiff_keeps_the_declared_sample_count_at_the_native_rate() {
    ...
    assert_eq!(packet.payload(), &[0; 2_048]);
    assert!(matches!(demux.read_packet(), Err(Error::Eof)));
}

#[test]
fn caf_keeps_the_declared_sample_count_at_the_native_rate() {
    let bytes = include_bytes!("fixtures/caf_44100_one_frame.caf");
    let mut demux = CafDemuxer::open(
        Box::new(MemorySource::new(bytes.to_vec())),
        &FormatOptions::default(),
    )
    .unwrap();
