...
fn real_tile_slice_header_has_one_tile_entry_point_offset() {
    ...
    let first_second_mpm_prefix = tile_states
        .get_mut(0)
        .expect("first tile state exists")
        .decode_first_ctb_leaf_second_mpm_idx_prefix(
            u32::from(sps.log2_min_cb_size),
            u32::from(sps.log2_min_cb_size),
        )
    ...
    let first_second_luma_mode = tile_states
        .get_mut(0)
        .expect("first tile state exists")
        .resolve_first_ctb_leaf_second_luma_mode(
            u32::from(sps.log2_min_cb_size),
            u32::from(sps.log2_min_cb_size),
        )
    ...
    let second_second_luma_error = tile_states
        ...
        .expect_err("second tile has no measured second PU MPM index");
    assert!(matches!(
        second_second_luma_error,
        Error::Unsupported("vaco-codec-hevc: first tile second PU MPM index is not the measured mode")
    ));
    ...
    let second_second_mpm_error = tile_states
        ...
        .expect_err("second tile has no second PU MPM prefix");
    assert!(matches!(
        second_second_mpm_error,
        Error::Unsupported("vaco-codec-hevc: first tile second PU has no explicit mpm_idx")
    ));
