    ...
    fn decode_packet(&mut self, pkt: &Packet) -> Result<()> {
        ...
        *first_start = hdr.slice_segment_address;
        ...
        if multiple_wpp_segments {
            ...
            {
                return Err(Error::Unsupported(
                    "vaco-codec-hevc: filtered WPP slice boundaries without cross-slice filtering are not supported",
                ));
            }
        }
        ...
        if pps.entropy_coding_sync_enabled {
            ...
            for (index, &ebsp) in slice_nals.iter().enumerate() {
                ...
                let (segment_qp, segment_kind, segment_cabac_init) = if parsed.dependent {
                    // §9.3.2.5: a dependent segment continues the
                    // preceding independent slice. WPP still resets
                    // each row's CABAC context under §9.3.2.3.
                    (active_slice_qp, active_slice_kind, active_cabac_init)
                } else {
                    let segment_qp = 26 + pps.init_qp_minus26 + parsed.qp_delta;
                    active_slice_qp = segment_qp;
                    active_slice_kind = parsed.kind;
                    active_cabac_init = parsed.cabac_init;
                    if index != 0 {
                    ...
                    walk.begin_slice_segment(
                        start_ctb,
                        slice_ends.get(index).copied().unwrap_or(end_ctb),
                        segment_qp,
                    );
...
/// Decode one tiles-only slice into tile-local CABAC substreams.
/// §7.4.7.1 measures every entry-point length in the escaped slice payload,
/// so each range is de-escaped independently before its fresh arithmetic
/// decoder is initialized. Tile scan consumes each substream top-to-bottom,
/// while the outer row walk suspends its CABAC/context state between CTU rows
/// so reconstruction, edge, CU and SAO publication all remain raster ordered.
#[allow(
...
fn decode_tile_substreams(
    ...
    let mut cabacs: Vec<_> = tile_data.iter().map(|data| CabacDecoder::new(data)).collect();
    let mut contexts = vec![new_context_bank(kind, cabac_init, qp); tile_data.len()];
    for row in 0..ctbs_y {
        ...
        for (tile_index, &(x0, x1, y0, y1)) in rectangles.iter().enumerate() {
            ...
            let cabac = cabacs.get_mut(tile_index).ok_or(Error::InvalidData(
                "vaco-codec-hevc: tile CABAC state is missing",
            ))?;
            let context = contexts.get_mut(tile_index).ok_or(Error::InvalidData(
                "vaco-codec-hevc: tile CABAC context is missing",
            ))?;
            ...
            for col in x0..x1 {
                ...
                let cx = i32::try_from(col).unwrap_or(0) * ctb_size_i;
                let cy = i32::try_from(row).unwrap_or(0) * ctb_size_i;
                ctu::decode_ctu(cabac, context, walk, cx, cy, addr)?;
                walk.recon.publish_ctu(row_us, col_us)?;
                let end = cabac.decode_terminate();
                if cabac.malformed() {
                    return Err(Error::InvalidData(
                ...
                let final_ctb = row.saturating_add(1) == y1 && col.saturating_add(1) == x1;
                let final_tile = tile_index.saturating_add(1) == rectangles.len();
                if final_ctb && !final_tile && end != 0 {
                    return Err(Error::InvalidData(
...
fn decode_wpp_single_column(
    ...
    let mut ctx = new_context_bank(kind, cabac_init, qp);
    let mut terminated = false;
    for row_idx in 0..row_count {
        let row_u32 = row_start.saturating_add(row_idx);
        let row_us = usize::try_from(row_u32).unwrap_or(usize::MAX);
        walk.qp_y_prev = walk.shared.slice_qp;
        walk.edges.begin_row(row_us)?;
        walk.cu_grid.begin_row(budget, row_us)?;
        walk.sao_params.begin_row(budget, row_us)?;
        walk.recon.begin_ctu(row_us, 0)?;
        let addr = row_u32.saturating_mul(walk.shared.ctbs_x);
        let cy = i32::try_from(row_u32).unwrap_or(0) * ctb_size_i;
        ctu::decode_ctu(&mut cabac, &mut ctx, walk, 0, cy, addr)?;
        walk.recon.publish_ctu(row_us, 0)?;
        if row_idx.saturating_add(1) == row_count {
            let end = cabac.decode_terminate();
            if cabac.malformed() {
                return Err(Error::InvalidData(
                    "vaco-codec-hevc: CABAC decode ran past the slice segment data",
