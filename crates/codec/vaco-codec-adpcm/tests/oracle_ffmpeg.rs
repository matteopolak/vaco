//! Independent fixtures for every "real" ADPCM decoder this
//! crate registers (`adpcm_ima_wav`, `adpcm_ima_qt`, `adpcm_ms`,
//! `adpcm_swf`, `g726`, and `g726le`) — the families this crate
//! had no external oracle for before this file. G.722 remains excluded because
//! it is unimplemented; the G.726 encoders likewise still refuse rather than
//! emit plausible but non-conforming data.
//!
//! # Why not a sine wave
//!
...
//! # Provenance
//!
//! Most fixtures use `Vaco-Provenance: blackbox`, `Vaco-Spec-Ref: none` —
//! measured directly against real `ffmpeg 9.0.1` output, not a cited
//! document. Each `tests/fixtures/<name>` container was produced by ffmpeg
//! 9.0.1 (`-f lavfi -i "aevalsrc=..."`, `-c:a <adpcm variant>`). Except for
//! G.726, each `tests/fixtures/<name>_ref.raw` is that same file decoded with
...
//!
//! G.726 is checked against the unrestricted-use original Sun G.72x reference
//! implementation, whose 32 kbit/s decoder matches all 16,384 outputs in the
//! official Appendix-II reset vector. The two short G.726 payloads are still
//! independently generated by ffmpeg, so they exercise real-world packing as
//! well as the standard's fixed-point decoder.
//!
//! IMA, MS, and G.726 ADPCM are fully-specified, integer-only adaptive
...
use vaco_codec_adpcm::{
    AdpcmG726Decoder, AdpcmImaQtDecoder, AdpcmImaWavDecoder, AdpcmMsDecoder, AdpcmSwfDecoder,
};
...
fn decode_g726_fixture(encoded: &str, reference: &str, low_nibble_first: bool) {
...
#[test]
fn g726_32_decodes_a_real_ffmpeg_stream_against_sun_reference() {
    decode_g726_fixture("g726_32.g726", "g726_32_ref.raw", false);
}

#[test]
fn g726le_32_decodes_a_real_ffmpeg_stream_against_sun_reference() {
    decode_g726_fixture("g726le_32.g726le", "g726le_32_ref.raw", true);
}

/// `g726_appii_i32.g726` is the full 16,384-word 32 kbit/s algorithm-stressing
/// input from ITU-T G.726 Appendix II's `DISK1/INPUT/I32`, packed two words per
/// byte in `g726` order. The freely published archive had SHA-256
/// `2af43f18c4a95e57ef137721d85988627973e8d5c0ee325cf30f755b70c46688`.
/// The linear PCM oracle is the unrestricted-use original Sun G.72x decoder.
/// That same acquired source produces every one of Appendix II's 16,384
/// published mu-law reset outputs exactly; the published `.O` file includes
/// G.711 companding and synchronous coding adjustment, which Annex A removes.
#[test]
fn g726_32_decodes_the_official_appendix_ii_i_input_bit_exact() {
    let payload = fixture("g726_appii_i32.g726");
    let reference = fixture("g726_appii_i32_ref.raw");
    assert_eq!(payload.len(), 8_192);
    assert_eq!(reference.len(), 32_768);
    decode_g726_fixture("g726_appii_i32.g726", "g726_appii_i32_ref.raw", false);
}
