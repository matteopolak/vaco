...
fn a(above_row: &[i32], i: i32) -> i32 {
    let idx = i + 1;
    usize::try_from(idx).ok().and_then(|i| above_row.get(i)).copied().unwrap_or(0)
}

fn l(left_col: &[i32], i: i32) -> i32 {
    usize::try_from(i).ok().and_then(|i| left_col.get(i)).copied().unwrap_or(0)
}
...
#[allow(clippy::too_many_arguments, clippy::too_many_lines, reason = "one dispatch over ten spec-defined modes")]
pub fn predict_intra(
    ...
    if mode == tables::V_PRED {
        for i in 0..size {
            for j in 0..size {
                pset(pred, size, i, j, a(above_row, i32::try_from(j).unwrap_or(0)));
            }
        }
        ...
        for i in 0..size.saturating_sub(1) {
            ...
            pset(pred, size, i, 0, round2(l(left_col, ii) + l(left_col, ii + 1), 1));
        }
        for i in 0..size.saturating_sub(2) {
            let ii = i32::try_from(i).unwrap_or(0);
            pset(pred, size, i, 1, round2(l(left_col, ii) + 2 * l(left_col, ii + 1) + l(left_col, ii + 2), 2));
        }
        if size >= 2 {
            pset(pred, size, size - 2, 1, round2(l(left_col, sz - 2) + 3 * l(left_col, sz - 1), 2));
        }
        for i in (0..size.saturating_sub(1)).rev() {
            for j in 2..size {
                let v = pget(pred, size, i32::try_from(i + 1).unwrap_or(0), i32::try_from(j).unwrap_or(0) - 2);
        ...
        for i in 0..size {
            for j in 0..size {
                ...
                let v = if ii + jj + 2 < sz * 2 {
                    round2(a(above_row, ii + jj) + a(above_row, ii + jj + 1) * 2 + a(above_row, ii + jj + 2), 2)
                } else {
                    a(above_row, 2 * sz - 1)
                };
                pset(pred, size, i, j, v);
        ...
        for i in 0..size {
            for j in 0..size {
                let (ii, jj) = (i32::try_from(i).unwrap_or(0), i32::try_from(j).unwrap_or(0));
                #[allow(clippy::integer_division, reason = "spec-defined: i/2, splitting a subblock row into an even/odd pair")]
                let half = ii / 2;
                let v = if ii & 1 != 0 {
                    round2(a(above_row, half + jj) + a(above_row, half + jj + 1) * 2 + a(above_row, half + jj + 2), 2)
                } else {
        ...
        for j in 0..size {
            ...
            pset(pred, size, 0, j, round2(a(above_row, jj - 1) + a(above_row, jj), 1));
        }
        if size >= 2 {
            pset(pred, size, 1, 0, round2(l(left_col, 0) + 2 * a(above_row, -1) + a(above_row, 0), 2));
            for j in 1..size {
                let jj = i32::try_from(j).unwrap_or(0);
                pset(pred, size, 1, j, round2(a(above_row, jj - 2) + 2 * a(above_row, jj - 1) + a(above_row, jj), 2));
            }
        }
        if size >= 3 {
            pset(pred, size, 2, 0, round2(a(above_row, -1) + 2 * l(left_col, 0) + l(left_col, 1), 2));
        }
        for i in 3..size {
            let ii = i32::try_from(i).unwrap_or(0);
            pset(pred, size, i, 0, round2(l(left_col, ii - 3) + 2 * l(left_col, ii - 2) + l(left_col, ii - 1), 2));
        }
        for i in 2..size {
            for j in 1..size {
                let v = pget(pred, size, i32::try_from(i).unwrap_or(0) - 2, i32::try_from(j).unwrap_or(0) - 1);
                pset(pred, size, i, j, v);
            }
        }
    } else if mode == tables::D135_PRED {
        pset(pred, size, 0, 0, round2(l(left_col, 0) + 2 * a(above_row, -1) + a(above_row, 0), 2));
        for j in 1..size {
            let jj = i32::try_from(j).unwrap_or(0);
            pset(pred, size, 0, j, round2(a(above_row, jj - 2) + 2 * a(above_row, jj - 1) + a(above_row, jj), 2));
        }
        if size >= 2 {
            pset(pred, size, 1, 0, round2(a(above_row, -1) + 2 * l(left_col, 0) + l(left_col, 1), 2));
        }
        for i in 2..size {
            let ii = i32::try_from(i).unwrap_or(0);
            pset(pred, size, i, 0, round2(l(left_col, ii - 2) + 2 * l(left_col, ii - 1) + l(left_col, ii), 2));
        }
        for i in 1..size {
            for j in 1..size {
                let v = pget(pred, size, i32::try_from(i).unwrap_or(0) - 1, i32::try_from(j).unwrap_or(0) - 1);
                pset(pred, size, i, j, v);
            }
        }
    } else if mode == tables::D153_PRED {
        pset(pred, size, 0, 0, round2(l(left_col, 0) + a(above_row, -1), 1));
        for i in 1..size {
            let ii = i32::try_from(i).unwrap_or(0);
            pset(pred, size, i, 0, round2(l(left_col, ii - 1) + l(left_col, ii), 1));
        }
        ...
        pset(pred, size, 0, 1, round2(l(left_col, 0) + 2 * a(above_row, -1) + a(above_row, 0), 2));
        pset(pred, size, 1, 1, round2(a(above_row, -1) + 2 * l(left_col, 0) + l(left_col, 1), 2));
        for i in 2..size {
            let ii = i32::try_from(i).unwrap_or(0);
            pset(pred, size, i, 1, round2(l(left_col, ii - 2) + 2 * l(left_col, ii - 1) + l(left_col, ii), 2));
        }
        for j in 2..size {
            let jj = i32::try_from(j).unwrap_or(0);
            pset(pred, size, 0, j, round2(a(above_row, jj - 3) + 2 * a(above_row, jj - 2) + a(above_row, jj - 1), 2));
        }
        for i in 1..size {
            for j in 2..size {
                let v = pget(pred, size, i32::try_from(i).unwrap_or(0) - 1, i32::try_from(j).unwrap_or(0) - 2);
                pset(pred, size, i, j, v);
            }
        }
...
mod tests {
    ...
    fn dc_pred_with_no_neighbours_is_the_bit_depth_midpoint() {
        ...
        predict_intra(&mut pred, tables::DC_PRED, 4, 2, &[0; 9], &[0; 4], false, false, 8);
    ...
    fn v_pred_copies_the_above_row() {
        ...
        let mut pred = [0i32; 16];
        predict_intra(&mut pred, tables::V_PRED, 4, 2, &above, &[0; 4], true, true, 8);
    ...
    fn h_pred_copies_the_left_column() {
        ...
        let mut pred = [0i32; 16];
        predict_intra(&mut pred, tables::H_PRED, 4, 2, &[0; 9], &left, true, true, 8);
    ...
    fn tm_pred_matches_the_formula() {
        ...
        let left = [20, 0, 0, 0];
        let mut pred = [0i32; 16];
        predict_intra(&mut pred, tables::TM_PRED, 4, 2, &above, &left, true, true, 8);
