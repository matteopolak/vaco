...
#[allow(clippy::integer_division, reason = "spec-defined: (value +/- 1) / 2, a rounding division, not a bug")]
fn round_mv_comp_q2(v: i32) -> i32 {
    (if v < 0 { v - 1 } else { v + 1 }) / 2
}

/// §8.5.2.1's `round_mv_comp_q4`.
#[allow(clippy::integer_division, reason = "spec-defined: (value +/- 2) / 4, a rounding division, not a bug")]
...
#[must_use]
pub(crate) fn select_mv(block_mvs: &[[i32; 2]; 4], block_idx: usize, plane: usize, mi_size_ge_8x8: bool, subsampling_x: bool, subsampling_y: bool) -> [i32; 2] {
    ...
    match (subsampling_x, subsampling_y) {
        ...
        (true, true) => {
            let sum: [i32; 2] = [0, 1].map(|c| (0..4).map(|i| get(i).get(c).copied().unwrap_or(0)).sum());
...
#[allow(clippy::too_many_arguments, reason = "mirrors the spec's own inputs, which Rust can't read off implicit globals")]
#[must_use]
pub(crate) fn clamp_mv(mv: [i32; 2], mi_row: usize, mi_col: usize, mi_rows: usize, mi_cols: usize, bw: usize, bh: usize, plane: usize, subsampling_x: bool, subsampling_y: bool) -> [i32; 2] {
    let (sx, sy) = if plane == 0 { (0u32, 0u32) } else { (u32::from(subsampling_x), u32::from(subsampling_y)) };
    ...
    let mb_to_bottom = ((i32::try_from(mi_rows).unwrap_or(0) - i32::try_from(bh).unwrap_or(0) - i32::try_from(mi_row).unwrap_or(0)) * tables::MI_SIZE * 16) >> sy;
    let mb_to_left = -((i32::try_from(mi_col).unwrap_or(0) * tables::MI_SIZE) * 16) >> sx;
    let mb_to_right = ((i32::try_from(mi_cols).unwrap_or(0) - i32::try_from(bw).unwrap_or(0) - i32::try_from(mi_col).unwrap_or(0)) * tables::MI_SIZE * 16) >> sx;
    let spel_left = (tables::INTERP_EXTEND + ((i32::try_from(bw).unwrap_or(0) * tables::MI_SIZE) >> sx)) << tables::SUBPEL_BITS;
    let spel_right = spel_left - tables::SUBPEL_SHIFTS;
    let spel_top = (tables::INTERP_EXTEND + ((i32::try_from(bh).unwrap_or(0) * tables::MI_SIZE) >> sy)) << tables::SUBPEL_BITS;
...
#[must_use]
#[allow(clippy::integer_division, reason = "spec-defined: (RefFrameWidth << REF_SCALE_SHIFT) / FrameWidth, an intentional fixed-point ratio, not a bug")]
pub(crate) fn scale_mv(clamped_mv: [i32; 2], x: usize, y: usize, plane: usize, subsampling_x: bool, subsampling_y: bool, ref_width: u32, ref_height: u32, frame_width: u32, frame_height: u32) -> ScaledMv {
    let x_scale = (i64::from(ref_width) << tables::REF_SCALE_SHIFT) / i64::from(frame_width.max(1));
    let y_scale = (i64::from(ref_height) << tables::REF_SCALE_SHIFT) / i64::from(frame_height.max(1));
    ...
    let luma_x = if plane > 0 { i64::try_from(x).unwrap_or(0) << u32::from(subsampling_x) } else { i64::try_from(x).unwrap_or(0) };
    let luma_y = if plane > 0 { i64::try_from(y).unwrap_or(0) << u32::from(subsampling_y) } else { i64::try_from(y).unwrap_or(0) };
    let frac_x = ((16 * luma_x * x_scale) >> tables::REF_SCALE_SHIFT) & i64::from(tables::SUBPEL_MASK);
    let frac_y = ((16 * luma_y * y_scale) >> tables::REF_SCALE_SHIFT) & i64::from(tables::SUBPEL_MASK);
...
/// (row-major) into `pred`.
#[allow(clippy::too_many_arguments, reason = "mirrors the spec's own inputs")]
#[allow(clippy::many_single_char_names, reason = "w/h/p/s/v/r/c/t are pixel coordinates, dimensions, filter-tap indices and accumulator values, matching the spec's own single-letter notation for this convolution")]
pub(crate) fn block_inter_predict(pred: &mut [i32], slot: &RefSlot, plane: usize, scaled: &ScaledMv, w: usize, h: usize, interp_filter: usize, bit_depth: u32) {
    let ref_p = ref_plane(slot, plane);
    let (sub_x, sub_y) = if plane == 0 { (false, false) } else { (slot.subsampling_x, slot.subsampling_y) };
    ...
    let intermediate_height = usize::try_from((((i64::try_from(h).unwrap_or(0) - 1) * scaled.step_y + 15) >> 4) + 8).unwrap_or(0);
    let clip_max = (1i32 << bit_depth) - 1;

    let filters = tables::SUBPEL_FILTERS.get(interp_filter).copied().unwrap_or(tables::SUBPEL_FILTERS[0]);
    ...
    for r in 0..intermediate_height {
        for c in 0..w {
            ...
            let taps = filters.get(phase).copied().unwrap_or([0, 0, 0, 128, 0, 0, 0, 0]);
            ...
            for (t, &tap) in taps.iter().enumerate() {
                ...
                let sample = ref_p.get_clamped(i32::try_from(col).unwrap_or(0), i32::try_from(row).unwrap_or(0));
