...
//! are resolved directly — by far the overwhelming majority of real AAC-LC
//! content, and the five configurations this workspace could confidently state the exact
//! `SCE`/`CPE`/`LFE` element ordering for without needing the ISO/IEC
//! 14496-3 Table 42 text on hand for the rarer ones (3, 7, 11, 12, 14)
...
/// program config element. See the module doc for why 3/7/11/12/14 are
...
impl DecoderConfig {
    ...
    pub fn from_adts_header(header: &AdtsHeader) -> Result<Self> {
        ...
        let channels = match known_channel_count(header.channel_configuration) {
            ...
            None => {
                return Err(Error::Unsupported(
                    "vaco-codec-aac: channel_configuration 3/7/11/12/14 are not \
