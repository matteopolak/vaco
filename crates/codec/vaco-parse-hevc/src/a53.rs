...
//! # What this is for
//!
//! CEA-608/708 captions reach an HEVC stream inside an SEI message of type 4
//! (§D.2.6), wrapped in the ATSC A/53 identification prefix. This module
...
//!
//! Two things differ from the H.264 carriage and neither changes this module:
//! an HEVC NAL header is two bytes rather than one, and prefix (39) and
//! suffix (40) SEI are separate NAL types — both are already handled by
//! [`crate::sei::parse`] before a payload reaches here. The payload type
//! number is 4 in both standards, which was checked rather than assumed.
//!
//! `cc_data()` itself (ANSI/CTA-708 Table 2, reproduced in ATSC A/53 Part 4
//! §6.2.3.1 Table 6.10, and carried in HEVC per ATSC A/72 Part 1) then
//! follows:
//!
//! ```text
...
//! contains. The equivalent H.264 path was verified byte-for-byte against
//! 361 frames of a real broadcast capture; this one is verified against a
//! transcode of that same capture (see the crate's test module), since no
//! natively-authored HEVC caption sample was obtainable.
//!
