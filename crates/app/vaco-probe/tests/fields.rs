...
fn a_video_stream_emits_the_table_in_order() {
    let got = keys(|e| {
        ...
        show::stream(e, &bare(0, MediaType::Video), true, show::Counts::NONE).expect("stream");
...
fn an_audio_stream_emits_the_table_in_order() {
    let got = keys(|e| {
        show::stream(e, &bare(0, MediaType::Audio), true, show::Counts::NONE).expect("stream");
    });
...
fn a_subtitle_stream_gets_width_and_height_and_nothing_else_visual() {
    let got = keys(|e| {
        show::stream(e, &bare(0, MediaType::Subtitle), true, show::Counts::NONE).expect("stream");
    });
    let got: Vec<String> = got.into_iter().take_while(|k| k != "default").collect();
...
fn the_packet_section_emits_its_table_in_order() {
    ...
    let got = keys(|e| {
        ...
        show::packet(e, &pkt, Some(&stream), show::PayloadOpts::default()).expect("packet");
...
fn every_declared_field_is_reachable_from_some_emitter() {
    ...
    for media in [MediaType::Video, MediaType::Audio, MediaType::Subtitle] {
        for k in keys(|e| {
            show::stream(e, &bare(0, media), true, show::Counts::NONE).expect("stream");
        }) {
            if !seen.contains(&k) {
                seen.push(k);
            }
