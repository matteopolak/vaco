...
/// NNN ...`.
pub fn parse_response(block: &str) -> Result<Response> {
...
mod tests {
    ...
    /// End-to-end against a real loopback `TcpListener` — this is the
    /// "captured bytes" substitute the crate docs describe: no live proxy is
    /// reachable, but a byte-for-byte real socket round trip is.
    #[test]
    fn dial_completes_against_a_local_listener_that_answers_200() {
        ...
        dial(&url, Some(Duration::from_secs(2)), &env).unwrap();
        handle.join().unwrap();
    }
