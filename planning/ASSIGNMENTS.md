...
| vaco-codec-h264 (epics #35, #37) | 3 | #418 #419 #420 #421 #424 | agent:h264-cabac | active | 2026-08-28 | — | CABAC syntax layer landed (4 real fixes); **no picture reconstruction exists at all** — `receive_frame` always returns `NeedMoreInput`. Now building the intra-only reconstruction (#424 dequant, then #420 intra pred) that gives #418 its first real oracle. Row was missing entirely through ten rounds of work — added by the orchestrator after the owning agent flagged it. |
| vaco-codec-dsp-deblock (new) | 3 | #127 then #425 | agent:h264-cabac | active | 2026-08-28 | — | crate does not exist yet; #425 (H.264 deblocking) is blocked on it. Scope: the D-10 measurement spike (masked-lane select) using the existing `vaco-checkasm` and `vaco-simd`, then the filter itself. Assigned to the consuming agent deliberately — it knows the interface it needs. |
