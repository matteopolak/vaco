...
| vaco-codec-h264 (epics #35, #37) | 3 | #418 #419 #420 #421 #422 #423 #424 #425 | agent:h264-cabac | active | 2026-08-28 | — | Intra reconstruction + deblocking byte-exact on five fixtures (noise/testsrc/multi/gradient/cabac_i_only, 99.78% on the one real-deblocking corpus). P-slice inter support (#421/#422/#423) landed this round as a batch (DPB, MV median prediction, six-tap qpel luma motion compensation) but not closed: `cabac_ip_simple.264` decodes end to end without crashing (new) at only 36% match against ffmpeg, with a real, time-boxed-but-unlocated defect specific to macroblock rows beyond the first (row 0 is byte-exact). `receive_frame` still always returns `NeedMoreInput` -- nothing built so far is wired to the public decoder entry point, only to this crate's own test fixtures. #418 stays open (own separate judgement posted): its own Acc needs every Table 9-34 context index exercised, and inter-prediction contexts remain unverified by any pixel-level method. |
