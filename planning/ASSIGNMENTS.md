...
| vaco-codec-h264 (epics #35, #37) | 3 | #418 #419 #420 #421 #422 #423 #424 | agent:h264-cabac | active | 2026-08-28 | — | #425 (deblocking) CLOSED under the owner's new bar (`705779d`): 99.78% on cabac_i_only, small unstructured table-rounding scatter, no P/B bS or chroma. #421/#422/#423 (P-slice inter) NOT closed: same new bar still disqualifies a structured defect -- row 0 of every frame byte-exact, every other row wrong, now narrowed twice (P_Skip mv-grid gap fixed, P_8x8 num_sub fill bug fixed, 36.15%->56.16%) but the row-0-vs-elsewhere shape persists, most likely in MV prediction for multi-way P_8x8 splits. `receive_frame` still always returns `NeedMoreInput`. #418 stays open: inter-prediction context coverage still the open question, reinforced not resolved by finding real bugs there this round. |
