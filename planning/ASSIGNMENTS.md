...
| vaco-codec-dv | 4 | #372 | — | free | — | — | Batched with #371/#377/#39 above; not started. Theora (#371) alone used up the batch's time budget once its real scope became clear, so DV was left honestly untouched rather than shipped shallow -- see the issue's own comment for a pointer to vaco-format-dv (frame format, already landed under agent:mpegps-dv) as the framing layer this should build on, and to the D6/D17 measure-against-ffmpeg methodology JPEG-LS and Theora both leaned on for gaps SMPTE 314M/370M (paywalled) don't otherwise cover. |
