...
| vaco-codec-theora (new) | 4 | #371 | agent:t2-batch | done | 2026-08-28 | 2026-08-28 | Theora decode, keyframes (FTYPE=0) only -- inter/delta frames return Error::Unsupported by design, not a residual bug (motion vectors + macro-block modes + whole/half-pixel prediction + reference-frame buffering would be a second, comparably-sized effort). Full intra pipeline: header parsing, coded-order Hilbert-curve geometry, 80-table DCT Huffman decode, DC prediction, dequant + the spec's exact integer IDCT, loop filter. No genuine Theora encoder was available in this environment to produce ground truth, so verification here is narrower than this tree's other from-scratch codecs -- spec-conformant by construction and checked against the spec's own worked numeric examples, not cross-verified against an independent decoder; documented plainly as a known gap in src/lib.rs. Found a genuine gap in the published spec PDF itself: section 6.4.1's loop filter limit table decode procedure is simply missing from the document (confirmed via page images, not a text-extraction artifact) -- reconstructed by analogy with the next, textually-complete section. Fixed a real bug in vaco-demux-ogg: the OggCodec::Theora branch built CodecParameters with no codec attached at all, so .ogv/Ogg-Theora could never resolve to any decoder. CodecId::Theora already existed in vaco-codec-core. |
