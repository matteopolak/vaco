...
## `vaco-mux-hds` (new, #618): HDS's fragments turned out not to be ISOBMFF at all — FLV tags in a bare `mdat`, not `moof`/`trun` — so less of `vaco-format-isom` transfers than `vaco-mux-smoothstreaming` hoped

New crate, epic #75, second child. Before writing anything, checked what
transfers from `vaco-format-isom` and from `vaco-mux-smoothstreaming`'s own
Smooth Streaming work, per this dispatch's explicit instruction. The answer
is more nuanced than "reuse the same fragment writers again": **HDS's own
fragment files are not ISOBMFF at all.** Byte-walking a real
`ffmpeg -f hds` reference's `stream0Seg1-Frag1` file found a single top-level
box, `mdat`, with no `moof` anywhere — its payload is a sequence of classic
FLV tags (the same `TagType`/`DataSize`/`Timestamp`/`StreamID`/payload/
`PreviousTagSize` shape a `.flv` file's own body has), audio and video
interleaved directly, each carrying an `AVCVIDEOPACKET`/`AACAUDIODATA`
payload wrapping the sample bytes. None of `vaco-format-isom::writer`'s
`mfhd`/`tfhd`/`trun`/`traf`/`moof` apply here — only its generic,
format-agnostic `build::{bx, fullbx}` box-header helpers do, reused for the
outer `mdat` (`flv.rs`) and for the `abst`/`asrt`/`afrt` bootstrap boxes
(`bootstrap.rs`), which are new, hand-written box types with no ISOBMFF
counterpart at all.

**The `abst` bootstrap box was measured field-by-field, not assumed**, per
this dispatch's own instruction that it is "the hard part" — a wrong
reading produces a file that parses cleanly and resolves to the wrong
fragment, not one that fails to parse. A real 122-byte reference `stream0.
abst` (one quality level, two fragments) was hex-dumped and decoded byte
offset by byte offset: `bootstrapinfoVersion`, a profile/live/update flags
byte, a **1000-tick (millisecond) timescale — not Smooth Streaming's
10,000,000-tick HNS**, `currentMediaTime`, one `asrt` naming segment 1 with
`fragmentsPerSegment`, and one `afrt` whose two entries'
`(firstFragment, firstFragmentTimestamp, fragmentDuration)` triples
reproduce the measured 10s/2.069s split exactly.
`bootstrap::tests::matches_the_measured_two_fragment_reference_exactly`
asserts the crate's own `build_abst` output against those 122 bytes,
hex string for hex string.

**A real, measured behavior this crate had to reproduce rather than
simplify away**: every fragment restates both tracks' sequence headers.
The reference's second fragment opens with a fresh AVC sequence header
(the `avcC` bytes verbatim) then a fresh AAC sequence header (the raw
`AudioSpecificConfig` verbatim), both timestamped at the fragment's own
start, before any real sample — `lib.rs::ensure_fragment_started`
reproduces this, checked in `tests/roundtrip.rs` against a real 12-second,
two-fragment mux.

**Gap 27 (`planning/INTERFACE-GAPS.md`, found closing #617) checked
against this format explicitly rather than assumed to recur**: it does
not apply. A two-quality-level reference tree (`stream0.abst`/
`stream1.abst`, each with its own fragment files) confirmed every file
HDS writes sits flat in the manifest's own directory — the same
convention `vaco-mux-dash`/`vaco-mux-hls` already use — with no
per-quality subdirectory the way Smooth Streaming's
`QualityLevels(<bitrate>)/` needs one. Recorded as a note on gap 27
itself so the next reader does not have to re-derive it.

**A quality level bundles video and audio into one file, not one file per
elementary stream**: measured directly (one video+one audio input
produces exactly one `stream0`, not a `stream0`/`stream1` pair; two
quality levels from four mapped streams produce exactly two). This
crate's own stream-to-level grouping rule — each `add_stream` fills the
first empty video/audio slot in the last-created level, or starts a new
level once that slot is already taken — is this crate's own explicit,
documented convention for driving the standard one-`add_stream`-per-call
`Muxer` interface into that shape; it is not a guess at `ffmpeg`'s own
internal grouping algorithm (never read, consistent with D7/D15), and it
happens to reproduce the exact grouping the reference's own multi-level
test run produced.

**No new external dependency for base64** (D10): the `Manifest`'s
`<metadata>` element is base64 AMF0. `base64.rs` hand-rolls RFC 4648 §4
encoding — the same amount of code, and the same convention, at least
four other crates in this workspace already use for the same reason
(`vaco-protocol-http::headers`, `vaco-protocol-local::data`,
`vaco-protocol-icecast::request`, `vaco-demux-rtsp`).

**Scoped out, named, not silently dropped**: re-framing Annex-B H.264 into
length-prefixed NALUs, and ADTS-framed AAC into raw access units — this
crate requires `VideoParameters::nal_length_size == Some(4)` and raw AAC
samples already, the same convention every MP4-family muxer in this
workspace already relies on, and refuses anything else with a clear error
(`tests/roundtrip.rs::a_stream_with_annexb_h264_is_rejected`) rather than
attempting a re-framing this crate does not implement. A second `abst`
segment / live sliding window (`-window_size`/`-extra_window_size`) is
also not implemented — every fragment lands in segment 1, matching every
reference tree measured (none produced a second segment at the durations
tested).

**Verification ceiling stated honestly**: issue #618's own Acc criterion —
"the manifest and fragment set match the reference's structure" — has no
"plays back through a client" clause the way #617's did, so this crate's
13 tests (10 unit, including the byte-exact `abst` match, plus 3
integration tests against real `file:` output via `WriteAccess`) meet the
issue's own stated bar directly, not a hedged subset of it.

Gates: `cargo test`/`cargo clippy -p vaco-mux-hds --all-targets -- -D
warnings` clean under the full workspace lint set (including
`indexing_slicing`, `unwrap_used`, `disallowed_methods`,
`integer_division` — every millisecond/kilobit unit conversion in this
crate goes through `vaco_core::Duration::to_ticks`'s own checked rescale
or, where the operands are already floats (AMF0's `/1024.0`, the
manifest's decimal `/1000.0`), plain float division, rather than a bare
integer `/`); builds for `wasm32-unknown-unknown`; `layer-check`/
`dep-gate`/`unsafe-audit`/`dup-check`/`owner-gate` all clean;
`cargo xtask gen-registry`/`gen-docs-index` both pick up the new crate
