...
## 69. HEVC on the JCT-VC subset: 2 of 46 byte-exact, not 23 broken streams but one off-by-one; and the suite's MP4 wrap loses content on 35 of 46

Measured 2026-09-03 against `ffmpeg 9.0.1` on this machine. Three separate
things, in the order they were established.

### The measurement the suite was not making

`tests/conformance/transcode/hevc-jctvc-conformance.toml` compares `vaco`
and `ffmpeg` on an MP4 wrap of each raw bitstream, generated by the
reference itself (`-fflags +genpts -i <stream> -c copy -f mp4`), because
`vaco`'s raw-elementary-stream demux assigns no packet timestamps. Running
that exact command on all 46 streams and then decoding **both** the wrap and
the original with `ffmpeg 9.0.1`: **35 of 46 wraps decode to a different
byte count than the bitstream they came from.** `rqt-a` 2 frames -> 1,
`poc-a` 5 -> 2, `rplm-a` 300 -> 95, `merge-a`..`merge-e` 8 -> 6, and
`cip-a-panasonic-3` to **zero** frames. `-f nut` (35 lossy, 33 of the copies
outright failing), `-f matroska` and `-f mpegts` (46/46, every copy
refused) are no better. The loss is in `-c copy` out of the Annex-B parser,
not in the container.

So a `jctvc-conformance` case that agrees, agrees on a subset of its stream,
occasionally an empty one. That is not a green suite reading green — the
suite reports its divergences honestly — but "42 diverged" and "4 agreed"
both mean less than they look like, and the 4 need saying out loud.

Every number below therefore comes from feeding each stream's raw Annex-B
access units straight to `HevcDecoder` and comparing every byte of every
plane against `ffmpeg -i <stream>.bin -f rawvideo -pix_fmt yuv420p`, with
the container out of the loop on both sides.

### 24 of the 46 desynced on one shared off-by-one

Before: **2 byte-exact, 24 CABAC desyncs** ("ran past the slice segment
data"), 6 wrong-pixel, 1 wrong-frame-count, 13 refused by name. The 24
desyncs were one bug. `ctu.rs` had transcribed HM's
`getQuadtreeTULog2MinSizeInCU` and fed it the SPS syntax element:
`MaxTrafoDepth` (H.265 7.4.9.8) is `max_transform_hierarchy_depth_*` plus
`IntraSplitFlag`/`interSplitFlag`, but HM *stores* that element plus one, so
its own code subtracts one back off and tests `interSplitFlag` against 1.
Both halves came out one too small, and the decoder stopped reading
`split_transform_flag` one transform level early — the bins the stream spent
on the deeper tree went unread, and every bin after them was misparsed.

A second, independent defect sat behind it: 8.5.3.2.1's closing step,
forcing a bi-predictive merge candidate for an 8x4 or 4x8 PU
(`nOrigPbW + nOrigPbH == 12`) to uni-prediction from L0, was absent.

Both were invisible to every fixture the crate had, for one measured reason:
a stock `libx265` invocation (SPS/PPS read back out of the encodes) writes
`max_transform_hierarchy_depth_inter/intra = 0` and `amp_enabled_flag = 0`
and leaves `--rect` off, which is exactly where the off-by-one coincides
with the correct value and where no 8x4/4x8 PU can exist. `tests/rqt.rs`
(commit `8b1e7f6`) is a `rect=1:amp=1:tu-inter-depth=3:tu-intra-depth=3:
bframes=2` encode that reaches both.

After: **25 byte-exact, 3 desyncs, 3 wrong-pixel, 2 wrong-frame-count, 13
refused.** `cip-a`, `cip-b`, `mvedge-a` and `picsize-d` — four streams
previously filed as separate wrong-pixel defects (#442, #431, #435) — are
byte-exact with no change of their own. `rplm-a` is byte-exact over 300
frames.

The prediction in the brief held exactly: one mechanism, 23-ish streams.

### What is left, named

- `initqp-a-sony-1`: desyncs after 47 of 60 frames. 60 PPSs, one per
  picture; every other stream in the subset has one.
- `slpplp-a-vidyo-2`, `vpsid-a-vidyo-2`: same 33-picture P-only content,
  both stopping after 7 frames. One VPS/SPS/PPS each, so not parameter-set
  switching. Not root-caused.
...
- `nooutprior-a-qualcomm-1` (50 frames vs 40), `nut-a-ericsson-5` (36 vs
  34): DPB flushing on `no_output_of_prior_pics_flag` / NAL type.
- Refused by name, not mis-decoded: `I_PCM` (5 streams), more than one
  slice segment per picture (6), custom scaling lists (2).

### Through the real binary, the container dominates

`vaco-conformance run --suite jctvc-conformance --tier full`, with the fixes
in: **46 cases, 4 agreed, 42 diverged.** 13 of the 42 are the refusals; 23
exit 183, of which only **2** are the CABAC desyncs above and **21** die in
`vaco-format-core::time::check_monotonic` before any pixel is compared —
the pictures the MP4 wrap dropped leave `dpb`'s bumping emitting POCs out of
order. Only 6 cases reach a byte comparison at all. The suite is
`tier = "full"`, so CI's `--tier core` job never selects it and nothing in
CI reports any of this.
