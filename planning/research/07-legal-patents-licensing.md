...
| **VC-1 / WMV3** | **Via LA** (successor to MPEG LA, which originated the pool). Confirmed live 2026-08-28: `via-la.com/licensing-programs/` still lists VC-1 as one of its current video programs, alongside AVC/H.264 and HEVC/VVC — it was not wound down when MPEG LA's other legacy programs (MPEG-2) effectively lapsed. <https://www.via-la.com/licensing-programs/vc-1/> | **Not stated as expired on the program page itself** (unlike MPEG-2's page, which explicitly notes its own last-patent-expired date — VC-1's page is silent on expiration, which this project reads as "not confirmed clear," not "confirmed clear"). The pool's own codec-sale royalty cap is stated through **2027**, and its participation-fee cap through **2022** — the asymmetry between the two cap end-dates was not investigated further this pass. | **Same distinction as AVC** — encoder and decoder are each their own "unit." First **100,000 units/yr free** (one affiliated legal entity), then $0.20/unit (100k–5M), $0.10/unit above 5M, enterprise cap ~$5.5M/yr (codec sales). A separate, lower-cardinality subscription-participation fee schedule also exists. | No | 🟡 **AMBER, pending an owner ruling — this row is a record of what was found, not a decision.** VC-1 was entirely absent from this document before 2026-08-28 despite `vaco-codec-vc1` existing in the tree; that gap is now closed, but no one with authority to accept the risk has reviewed it. The shape is structurally identical to AVC's own row (same pool family, same unit-based free tier, same enterprise cap pattern) with materially less research depth behind it — treat this as a placeholder that unblocks `default = false` registration, not as equivalent diligence to the AVC row above it. |
